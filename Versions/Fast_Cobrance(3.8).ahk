...
Final_msg(Cred, Var, Urgence){
	...
	Send("{Enter}")
		...
		if(nome.Result != 'Cancel'){
			...
			Send("*⚠️Urgente⚠️*")
			Send("{Shift down}{Enter}{Shift up}")
			Send("^aBom dia/Boa tarde {Text}Sr(a). " nome.Value)
			Send("{Shift down}{Enter}{Shift up}")
...
::btv::{
	...
	if(variação.Result != 'Cancel' && nome.Result != 'Cancel'){
		...
		WinWaitNotActive('Escolha Credor')
		Winactivate("ahk_exe chrome.exe")
		Send("^aOlá, Boa tarde " nome.Value "?")
...
::sloc::Não localização da fachada no Google Maps 
::locn::Localizado numero novo
::locf::Localizado fachada
::locem::Localizado e-mail

	...
	if (Cred){
		...
		loop Configs.Length {
			...
			if(InStr(line, "<cred>") != 0){
				...
				;MsgBox(line)
				list_cred := StrSplit(line, ",")
				;MsgBox(list_cred[1])
				Credores.Push(list_cred)
			}
		}
		;MsgBox(Credores)
		return Credores[1]
...
::parc sm::{
	vezes := InputBox('Em quantas vezes o parcelamento?', 'Parcelamento')
	valor_p := InputBox('Qual será o valor das parcelas?', 'Parcelamento')

	if (vezes.Result != 'Cancel' && valor_p.Result != 'Cancel'){
		Winactivate("ahk_exe chrome.exe")
		Parcelamento(vezes.Value, valor_p.Value, 7, 0)
	}
	else{
		MsgBox('Automação cancelada', 'Cancelada')
	}
...
::parc qz::{
	...
	if (vezes.Result != 'Cancel' && valor_p.Result != 'Cancel'){
		Winactivate("ahk_exe chrome.exe")
		Parcelamento(vezes.Value, valor_p.Value, 15, 0)
	}
	else{
		MsgBox('Automação cancelada', 'Cancelada')
	}
...
::parc m::{
	...
	if (vezes.Result != 'Cancel' && valor_p.Result != 'Cancel'){
		Winactivate("ahk_exe chrome.exe")
		Parcelamento(vezes.Value, valor_p.Value, 30, 0)
	}
	else{
		MsgBox('Automação cancelada', 'Cancelada')
	}
...
::parc x::{
	vezes := InputBox('Em quantas vezes o parcelamento?', 'Parcelamento')
	valor_p := InputBox('Qual será o valor das parcelas?', 'Parcelamento')
	interval := InputBox('Qual será o intervalo entre as parcelas?', 'Parcelamento')

	if (vezes.Result != 'Cancel' && valor_p.Result != 'Cancel' && interval.Result != 'Cancel'){
		Winactivate("ahk_exe chrome.exe")
		Parcelamento(vezes.Value, valor_p.Value, interval.Value, 0)
	}
	else{
		MsgBox('Automação cancelada', 'Cancelada')
	}
...
::parc d::{
	...
	valor_p := InputBox('Qual será o valor das parcelas?', 'Parcelamento')
	interval := InputBox('Qual será o intervalo entre as parcelas?', 'Parcelamento')
	desconto := InputBox('Quanto de desconto sobre os juros?', 'Parcelamento')

	if (vezes.Result != 'Cancel' && valor_p.Result != 'Cancel' && interval.Result != 'Cancel' && desconto.Result != 'Cancel'){
		Winactivate("ahk_exe chrome.exe")
		Parcelamento(vezes.Value, valor_p.Value, interval.Value, desconto.Value)
	}
	else{
		MsgBox('Automação cancelada', 'Cancelada')
	}
...
::avs::{
	valor := InputBox("Qual o valor do boleto a vista hoje?", 'Boleto a vista hoje')
	if (valor.Result != 'Cancel' ){
		Winactivate("ahk_exe chrome.exe")
		A_Vista(valor.Value, 0)
	}
	else{
		MsgBox('Automação cancelada', 'Cancelada')
	}
...
::avd::{
	valor := InputBox("Qual o valor do boleto a vista hoje COM DESCONTO?", 'Boleto a vista hoje')
	desconto := InputBox('Quanto de desconto sobre os juros? (%)', 'Boleto a vista hoje')
	if (valor.Result != 'Cancel' && desconto.Result != 'Cancel'){
		Winactivate("ahk_exe chrome.exe")
		A_Vista(valor.Value, desconto.Value)
	}
	else{
		MsgBox('Automação cancelada', 'Cancelada')
	}
...
::qbrd::
{
	...
	if(nome.Result != 'Cancel'){
		Choice_Cred.show('w240 h180')
		WinWaitNotActive('Escolha Credor')
		Send("^a{Text}Olá Sr(a). " nome.Value ", não identificamos o pagamento do acordo que formalizamos para pagamento ontem referente aos débitos da " Credores[Escolha.value] ", o que ocorreu?")
		Send("{Enter}")
	}
	else {
		MsgBox('Automação cancelada', 'Cancelada')
...
::qbrz::
{
	...
	if(nome.Result != 'Cancel'){
		Choice_Cred.show('w240 h180')
		WinWaitNotActive('Escolha Credor')
		Send("^a{Text}Olá Responsável da razão social: " nome.Value ", não identificamos o pagamento do acordo que formalizamos para pagamento ontem referente aos débitos da " Credores[Escolha.value] ", o que ocorreu?")
		Send("{Enter}")
	}
	else {
		MsgBox('Automação cancelada', 'Cancelada')
...
::prpet::{arquiv}Repassado proposta para entrada hoje
